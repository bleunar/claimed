...
*   **Computer Sets**: A group of hardware (e.g., "PC 16", which includes a CPU, Monitor, and Keyboard).
*   **Components**: The actual hardware items (Keyboard, Monitor, Mouse, CPU Units, etc.).

## "Assigned" vs "Rogue" Components
*   **Assigned**: A component that is part of a **Computer Set**.
*   **Rogue (Unassigned)**: A component that is not currently assigned to any specific computer set. Check the **Rogue Only** filter on the Components page to find components that are not assigned to a specific group.
